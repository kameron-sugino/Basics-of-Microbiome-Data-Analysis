---
title: "20200324_Workshop_Code_ARCHBG_Markdown"
...
* The ARCHBG data
  * I should have emailed this to you already

# 1) Introduction

* This code is meant to be a primer on R code that is specifically geared towards how to implement and understand the Comstock lab's microbiota analysis methods. There are many things I will not cover here, R code that I don't really understand fully and things that I just won't explain very well. For all of these things (and more!), there is always Google.
...
* Anyway, let's start by installing the packages you'll need for this.
  * Packages in R are meant to make your job easier. They are pre-written lines of code meant to do a specific job without you needing to code (or fully understand) how to implement your method of interest.
...
* Before we get to the data, let's make sure R knows to use the packages we just installed.
...
```{r}
setwd('D:\\20200122_Workshop_Code\\')
Data.Raw<-read.table('Data.Raw.txt',fill=TRUE,header=TRUE)
```
...
* The next line reads in the data table from the directory I specified above, so it's looking for the file names "Data.Raw.txt" in the folder "20200122_Workshop_Code".
...
* The first three columns contain info on the "label", "Group" and "numOtus", while the rest of the columns contain the number of reads (i.e. counts) for each OTU. Briefly, an OTU stands for "Operational Taxonomic Unit" and I will be using it to refer to the bacterial counts in our dataset.
  * The label column tells you what taxonomic level you are looking at
    * This dataset contains the OTU information from Kingdom to Genus
  * Group tells you the sample info. 
    * In this case, our IDs are labeled as "ID#-timepoint", where "1w" are infant samples and "M" are the maternal samples.
  * numOtus is just the number of OTU columns there are in this dataset. It's telling us that there are 439 uniquely identified OTUS
...
* I'm pretty bad at explaining things like this, so I'm sure you're still very confused. Sorry. You'll get the hang of it as you use R more, I promise.

* Anyway, we want the genus-level info, so we're looking for the rows with a "1" under the "label" column.
...
write.table(Data.Subsample, 'D:\\Data.Subsample.txt', quote=FALSE, row.names=FALSE)
```
...
write.table(Data.Baby1w, 'D:\\Data.Subsample.Baby1w.txt', quote=FALSE, row.names=FALSE)
#Mom samples#
Data.Mom<-Data.Subsample[grepl("M|m", Data.Subsample$Group),]
write.table(Data.Mom, 'D:\\Data.Subsample.Mom.txt', quote=FALSE, row.names=FALSE)
```
...
```{r}
BCData$BMI_Category_Final<-factor(BCData$BMI_Category_Final,levels(BCData$BMI_Category_Final)[c(1,3,2)])
...
```{r}
setwd('D:\\20200122_Workshop_Code\\')
TaxName<-read.table("fileList.paired.trim.contigs.good.unique.good.filter.unique.nr_v128.wang.pick.tx.1.cons.txt",header=TRUE, fill=TRUE,row.names=NULL)
...
* Great, looks like some bacterial names, right? You'll notice some are named "unclassified". It just means that the OTU couldn't be identified to the genus level, only to the family level.

# 6) Stacked Barplot of OTU Abundances

* So, what's this data look like? Let's take a look at it in the most overwhelming way possible, with the StackedBarPlot() function:
...
* Let's say you want to make a table out of this with the means and standard deviation. 
  * A reviewer asked us to remove a single participant from out analysis, *so I had to redo all of the tables manuall**.
...
* Let's do beta diversity now. There's one function for both Sorensen (presence/absence) and Bray-Curtis (abundance) dissimilarities and to produce the PCoA plot.
...
Sor.bray.pcoa<-function(OTUS,Dim=2,Color=1,binary,Title="PCoA",...){
  ...
  plot(Data.df.PCoA, col=Color,
       main=Title,xlab=xlab,ylab=ylab,pch=c(pch),...)
  return(Data.df.PCoA)
}
```
...
```{r}
PERMANOVA(Data.Baby1w[,-c(1:3)],BCData$BMI_Category_Final,TRUE,9999)
PERMDISP(Data.Baby1w[,-c(1:3)],BCData$BMI_Category_Final,TRUE,9999)
```
...
PERMANOVA(Data.Baby1w[,-c(1:3)],BCData$Exclusive_BF,TRUE,9999)
PERMDISP(Data.Baby1w[,-c(1:3)],BCData$Exclusive_BF,TRUE,9999)
```
...
```{r}
png("Sor.Baby1w.BMI.png", res=300, height=7, width=7.5, units="in", compression="zip")
plot(df.Baby1w.Sor,cex.axis=1.5,cex.lab=1.5,cex.main=1,col=1,
     pch=21,cex=2,xlim=c(-.3,.55),ylim=c(-.35,.35),xlab="PC1 (16.8%)",ylab="PC2 (13.9%)",bg=Color.Baby)
ordiellipse(df.Baby1w.Sor,BCData$BMI_Category_Final,col=c("#000000","#E79F00","#0072B2"),lwd=2)
legend(.25,-.09,c("Normal","Overweight","Obese"), 
...
PERMANOVA(Data.Baby1w[,-c(1:3)],BCData$BMI_Category_Final,FALSE,9999)
PERMDISP(Data.Baby1w[,-c(1:3)],BCData$BMI_Category_Final,FALSE,9999)
```
...
PERMANOVA(Data.Baby1w[,-c(1:3)],BCData$Exclusive_BF,FALSE,9999)
PERMDISP(Data.Baby1w[,-c(1:3)],BCData$Exclusive_BF,FALSE,9999)
```
...
* What this basically means is your taxa distribution is treated as Poisson (another type of distribution that is used to interpret count data).
  * It's really a problem when this occurs, it's just telling you your data distribution is not what it expected.
...
* Wow. Amazing. This graph is telling you what bacterial taxa are significantly different between "Normal" and the other two BMI categories. 
...
```{r}
setwd("D:\\2016 Sample Data\\Wheat_Study\\20190925_Mothur_Processed_Sequences")

Data<-read.table("Data.Subsample.Genus.shared",header=TRUE)
Data<-Data[-c(109:110,29),]
TaxName<-read.table("stability.trim.contigs.good.unique.good.filter.unique.precluster.pick.nr_v132.wang.pick.tx.1.cons.taxonomy",header=TRUE)
...
meta<-read.csv("D:\\2016 Sample Data\\Wheat_Study\\MIWh Sample Info Forms_October 15, 2019_12.29.csv",header=TRUE)
...
bubble.plot<-function(OTUs_group1,OTUs_group2,ids_group1,ids_group2){
  ...
  plot(y=as.integer(df$variable), x=as.integer(df$ids), pch=circles, cex=size, bty="n", axes = F, xlab="", ylab="")
  axis(2, at = unique(as.integer(df$variable)), labels = levels(df$variable), line = 0.5, las=2)
  axis(3, at = unique(as.integer(df$ids)), labels = levels(df$ids), line = 0.5,las=2)
